...
| P8  | E2E + internal audit        | mixed                | 🟡 in progress | P5, P6, P7 | full vanilla-anvil lifecycle + replay + 2C penalty drill + 2D key rotation in `e2e/src/scenarios.test.ts`; fork e2e + USDC whale wired in CI; DeepSeek audit reconciled in [`docs/audit-status.md`](./docs/audit-status.md). [08-e2e-and-audit.md](./docs/plans/08-e2e-and-audit.md) |
| P9  | Ops & infra                 | `[human]` heavy      | 🟡 in progress | —         | Fly manifests in `infra/fly/`, Prometheus + Grafana + Alertmanager in `infra/monitoring/`, watchtower litestream + restore drill in `infra/scripts/restore-drill.sh`. Remaining: tagged release deploy, paging test, restore-drill green run. [09-ops.md](./docs/plans/09-ops.md) |
| P10 | Mainnet real-money test     | `[human]` heavy      | 🟡 in progress | P9        | Operator-wallet decisions locked; ownership-transfer scripts ready (`packages/contracts/script/{Deploy,Transfer}Timelock.s.sol`); smoke channel + multisig handoff + external audit gated in [`docs/launch-checklist.md`](./docs/launch-checklist.md). [10-launch.md](./docs/plans/10-launch.md) |
