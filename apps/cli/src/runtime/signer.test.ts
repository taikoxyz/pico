...
import { encryptPrivateKey, serializeKeyFile } from '@tainnel/sdk';
...
describe('resolveSigner', () => {
  ...
  it('uses TAINNEL_PRIVATE_KEY env (warns)', async () => {
    const stderr = new StubStderr();
    const s = await resolveSigner({ env: { TAINNEL_PRIVATE_KEY: PK }, stderr });
    expect((await s.address()).toLowerCase()).toBe(ADDR);
    expect(stderr.buf).toContain('warn');
  });

  it('falls back to encrypted key file with passphrase prompt', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'tainnel-sig-'));
  ...
  it('errors when no source available', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'tainnel-sig-'));
    await expect(
      resolveSigner({ env: { TAINNEL_CONFIG_DIR: dir }, stderr: new StubStderr() }),
...
describe('resolvePrivateKey', () => {
  ...
  it('returns the private key from env var', async () => {
    expect(await resolvePrivateKey({ env: { TAINNEL_PRIVATE_KEY: PK } })).toBe(PK);
  });

  it('decrypts an encrypted key file using TAINNEL_PASSPHRASE', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'tainnel-sig-'));
    const keyFile = join(dir, 'key.enc');
    const env = encryptPrivateKey(PK, 'pw', FAST);
    writeFileSync(keyFile, serializeKeyFile(env), { mode: 0o600 });
  ...
  it('errors when env var malformed', async () => {
    await expect(resolvePrivateKey({ env: { TAINNEL_PRIVATE_KEY: 'not-hex' } })).rejects.toThrow();
  });
});
